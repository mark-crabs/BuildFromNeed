pub mod types;
use self::types::{Claims, TokenBody};
...
use oauth2::{AuthorizationCode, ClientId, ClientSecret, PkceCodeVerifier, RedirectUrl, TokenUrl};
...
> {
    ...
    let client = BasicClient::new(ClientId::new(oauth.client_id.clone()))
        ...
        .set_token_uri(
            TokenUrl::new(format!("{}/oauth2/token", oauth.domain))
                .map_err(actix_web::error::ErrorInternalServerError)?,
        )
        .set_redirect_uri(
            RedirectUrl::new(token_body.redirect_uri.clone())
...
pub async fn validate_auth0_token(
    ...
    let n = key["n"]
        .as_str()
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Invalid key"))?;
    let e = key["e"]
        .as_str()
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Invalid key"))?;

    let decoding_key = DecodingKey::from_rsa_components(n, e)
        .map_err(|_| actix_web::error::ErrorInternalServerError("Failed to construct key"))?;
